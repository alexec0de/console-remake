...
class MYSQL:
    ...
    def session(self):
        ...
        try:
        	connection = connect(host=self.dbhost,port=self.dbport,user=self.dbuser,password=self.dbpwd,db=self.dbname,charset=self.dbcharset,cursorclass=cursors.DictCursor)
        	self.connectdb = True
        	print(' * Connect on DB: OK')
        except:
        	print(' * Connect on DB: ERROR')	
        return connection
